...
<template>
  <header class="bg-white p-6 md:p-8">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div class="hidden md:flex flex-col gap-2">
          <h1
            class="text-2xl font-semibold leading-[150%] tracking-[-3%] text-[#141522]"
          >
            {{ pageHeader || "" }}
          </h1>
        </div>
    ...
    <div
      v-if="hasSearch"
      class="mt-6 w-full flex items-center justify-between sm:gap-8 gap-6"
    >
      <div class="flex-1 sm:max-w-105">
        <Input
          v-model="searchQuery"
      ...
      <div class="hidden sm:flex items-center justify-end gap-4">
        <button
          class="flex items-center gap-2.5 px-4 py-3.5 rounded-[10px] border border-gray-300 bg-white hover:bg-gray-50 transition-all cursor-pointer whitespace-nowrap"
        >
        ...
        >
          <FilterIcon />
          <span class="text-xs text-[#141522] font-medium"
            >Sort By : Deadline</span
          >
        </button>
      </div>

      <button
        class="flex sm:hidden items-center gap-2.5 px-3.25 py-3.25 rounded-[10px] border border-gray-300 bg-white hover:bg-gray-50 transition-all cursor-pointer whitespace-nowrap"
      >
        <FilterIcon size="24" />
      </button>
    </div>
