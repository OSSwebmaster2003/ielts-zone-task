...
<template>
  <div class="bg-white rounded-[10px] sm:p-6 p-4 flex flex-col gap-5 w-full h-full">
    <div class="flex items-center justify-between gap-3 flex-wrap">
      ...
      <div class="">
        <button v-if="!followed" @click="$emit('follow')"
        ...
        <span v-else class="text-[#54577A] text-sm whitespace-nowrap leading-[150%] tracking-[-2%] font-medium">
          Followed
        </span>
      </div>
    </div>
    ...
    <div class="flex items-center flex-wrap sm:gap-x-8 gap-x-4 gap-y-2 text-xs">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-[#141522] font-medium text-sm leading-[150%] tracking-[-2%]">({{ reviews }} reviews)</span>
      </div>
    </div>
  </div>
</template>
